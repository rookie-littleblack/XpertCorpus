...
class FileStorage(XpertCorpusStorage):
    ...
    def read(self, output_type: Literal["dataframe", "dict"]) -> Any:
        ...
        xlogger.info(f"Reading data from {file_path} with type {output_type}")

        if self.operator_step == 0:
            source = self.first_entry_file_name
            xlogger.info(f"Reading remote dataset from {source} with type {output_type}")
    ...
    def write(self, data: Any) -> Any:
        ...
        xlogger.success(f"Writing data to {file_path} with type {self.cache_type}")
