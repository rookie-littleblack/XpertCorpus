"""
This operator is used to define the Registry.
        ...
        if ret is None:
            if self._name == 'operator': 
                for x in ['eval', 'generate', 'process', 'refine']:
                    module_path = "xpertcorpus.modules.operators." + x
                    try:
                        ...
                        raise e
                xlogger.error(f"No object named '{name}' found in '{self._name}' registry!")
...
class LazyLoader(types.ModuleType):
    ...
    def _load_class_from_file(self, file_path, class_name):
        ...
        try:
            ...
            xlogger.debug(f"LazyLoader {self.__path__} successfully imported spec {spec.__str__()}")
            module = importlib.util.module_from_spec(spec)
            xlogger.debug(f"LazyLoader {self.__path__} successfully imported module {module.__str__()} from spec {spec.__str__()}")
            spec.loader.exec_module(module)
        except Exception as e:
            xlogger.error(f"{e.__str__()}")
            raise e
    ...
    def __getattr__(self, item):
        ...
        if item in self._loaded_classes:
            ...
            xlogger.debug(f"Lazyloader {self.__path__} got cached class {cls}")
            return cls
        # 从映射结构中获取文件路径和类名
        if item in self._import_structure:
            file_path, class_name = self._import_structure[item]
            xlogger.info(f"Lazyloader {self.__path__} trying to import {item} ")
            cls = self._load_class_from_file(file_path, class_name)
            xlogger.debug(f"Lazyloader {self.__path__} got and cached class {cls}")
